...
class PairingsTableController:
    ...
    def display_pairings(
        ...
        if bye_players:
            ...
            if bye_players:
                self.lbl_bye.setText(self._build_bye_text(bye_players))
    ...
    def setup_columns(self) -> None:
        """Configure column resize modes and widths. Call once after construction."""
        header = self.table.horizontalHeader()
        header.setSectionResizeMode(0, QtWidgets.QHeaderView.ResizeMode.Fixed)
        header.setSectionResizeMode(1, QtWidgets.QHeaderView.ResizeMode.Stretch)
        header.setSectionResizeMode(2, QtWidgets.QHeaderView.ResizeMode.Stretch)
    ...
    def _build_bye_text(self, bye_players: List[Player]) -> str:
        """Format the bye info bar text for one or more bye players."""
        if len(bye_players) == 1:
            ...
            return f"{player.name} ({player.rating}){status} receives {score} point"

        active_byes = [p for p in bye_players if p.is_active]
        inactive_byes = [p for p in bye_players if not p.is_active]

        details = [
            f"{p.name} ({p.rating}){' (Inactive)' if not p.is_active else ''}"
            for p in bye_players
        ]
        text = ", ".join(details)

        if active_byes and inactive_byes:
            text += f" — {len(active_byes)} receive {BYE_SCORE}pts, {len(inactive_byes)} receive 0pts"
        elif active_byes:
            text += f" — Each receives {BYE_SCORE} point{'s' if BYE_SCORE != 1 else ''}"
        else:
            text += " — Each receives 0 points"
